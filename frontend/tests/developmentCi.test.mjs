...
const event = { name: 'pull_request', base: 'main', head: 'eng/architectural-property-workspace', repository: 'khalilbilleh89/Reach-Developments-Station', headRepository: 'khalilbilleh89/Reach-Developments-Station' };
...
test('Main pushes, forks and other PR branches keep normal CI', () => {
  for (const change of [{name:'push'}, {base:'integration/mvp3'}, {head:'eng/another-change'}, {head:'eng/development-ui-delivery'}, {head:'eng/visual-02-development-delivery'}, {headRepository:'fork/repo'}, {repository:'other/repo'}]) assert.equal(isDevelopmentUiOnly({...event,...change}, ['frontend/src/app/globals.css']), false);
});
