...
import { CountComposition, CountSeries, Disclosure, Button, SectionHeader, KeyValueGrid, KeyValue, StatusDot, Field, FieldRow, FormActions, Loading, Meter, Notice, Position, PositionFigure, Tabs, TabPanel, TableScroll } from "@/components/ui";
...
// A ratio the server reported, drawn at its own width with the counts it came
// from beneath: a bar alone is not a number anybody can quote.
const ratio = (value: Ratio) => value.percentage === null ? "Unavailable" : <Meter neutral percent={value.percentage} note={`${value.numerator} / ${value.denominator}`} />;
// One status dimension as the server counted it, in the words and tones the
// inventory register already uses for the same states.
const dimension = (values: Record<string, number>) => Object.entries(values).map(([status, count]) => ({ label: statusLabel(status), count, tone: statusTone(status) }));
