...
export function ProjectMilestones({projectId,roles,refreshKey,onNavigate}: {projectId:string;roles:Roles;refreshKey:number;onNavigate:(section:"permits"|"consultant")=>void}) {
  ...
  return <section className="briefing-dates" aria-label="Dates to watch"><header><div><span className="eyebrow">Development diary</span><h2>Dates to watch</h2></div><span>{loading ? "Loading dates…" : dates.length + " available dated items"}</span></header>
