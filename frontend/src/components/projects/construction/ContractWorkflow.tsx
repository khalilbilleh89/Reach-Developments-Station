...
import { Button, ButtonRow, Card, DraftBoundary, RecordPage, Field, FieldRow, FormDialog, KeyValue, KeyValueGrid, MoneyInput, Notice, RateInput, TableScroll } from "@/components/ui";
...
export function ContractHeaderEditor({ detail, currencyId, currencyCode, busy, failure, unavailable = false, onSubmit, onCancel }: HeaderProps) {
  ...
  function submit() {
    ...
    onSubmit({ ...terms, ...(!detail ? { signed_contract: true, signed_reference: signedReference } : {}), currency_id: currencyId, contract_number: form.contract_number.trim(), vendor_name: form.vendor_name.trim(), retention_rate_fraction: fractionFromPercent(retention), tax_rate_fraction: tax.trim() ? fractionFromPercent(tax) : null, planned_start_date: form.planned_start_date || null, planned_completion_date: form.planned_completion_date || null });
  }
  return <RecordPage title={detail ? "Edit draft terms" : "Add signed contract"} onClose={onCancel}>
    <DraftBoundary dirty={JSON.stringify(form) !== JSON.stringify(initialForm) || !!signedReference} busy={busy}>
    <form className="stack" onSubmit={event => { event.preventDefault(); if (!busy && !unavailable) submit(); }}>
    {failure ? <Notice tone="error">{failure.message}</Notice> : null}
    <p>{detail ? "Update these draft terms, then activate the signed agreement from Contract details." : "Enter the agreement already signed with your main contractor or subcontractor. Saving activates it immediately; no budget is needed."}</p>
    {!detail ? <Field label="Signed contract document reference"><input className="input" required maxLength={500} value={signedReference} onChange={e => setSignedReference(e.target.value)} /></Field> : null}
    ...
    <Field label="Main contractor / subcontractor name" error={fieldError(failure, "vendor_name")}><input className="input" required maxLength={200} value={form.vendor_name} onChange={e => change("vendor_name", e.target.value)} /></Field>
    ...
    <FieldRow><Field label="Retention percentage" error={fieldError(failure, "retention_rate_fraction")}><RateInput required value={form.retention} onChange={v => change("retention", v)} /></Field><Field label="Tax percentage" optional hint="Enter the agreed tax rate to show the remaining balance including tax. Enter zero for no tax; blank leaves that balance unavailable." error={fieldError(failure, "tax_rate_fraction")}><RateInput value={form.tax} onChange={v => change("tax", v)} /></Field></FieldRow>
    ...
    <Button type="submit" variant="primary" disabled={busy || unavailable || !currencyId}>{busy ? "Saving…" : detail ? "Save draft" : "Save and activate"}</Button>
    </form></DraftBoundary>
  </RecordPage>;
}
...
const labels = { signed: "Activate signed contract", submit: "Submit for authorization", activate: "Authorize and activate", cancel: "Cancel uncommitted contract", complete: "Complete contract", terminate: "Terminate contract" };
...
function ContractDecision({ action, busy, failure, unavailable = false, onSubmit, onCancel }: { action: Action; busy: boolean; failure: Error | null; unavailable?: boolean; onSubmit: (body: Record<string, unknown>) => void; onCancel: () => void }) {
  ...
  const needsReason = action === "cancel" || action === "terminate" || action === "signed";
  return <FormDialog title={labels[action]} confirmLabel={labels[action]} busy={busy} disabled={unavailable || (needsReason && !reason.trim())} onSubmit={() => onSubmit({ reason: reason.trim() })} onCancel={onCancel}>
    {failure ? <Notice tone="error">{failure.message}</Notice> : null}
    <p>{action === "signed" ? "Confirm this agreement has already been signed and enter its document reference. It becomes active immediately without a budget or another approval." : action === "submit" ? "Freeze the terms and lines for independent authorization. Correct submitted mistakes by cancelling this uncommitted record with a reason and preparing a new draft." : action === "activate" ? "Activate this draft contract after checking its terms, exact line totals and currency. No budget is required." : action === "cancel" ? "Keep the uncommitted record and its lines as cancelled history. Enter why it is being abandoned. This action cannot cancel a live commitment." : "Record the end of this contract. Its financial commitment remains unchanged; reducing an obligation requires an approved variation."}</p>
    {needsReason ? <Field label={action === "signed" ? "Signed contract document reference" : "Reason"} error={fieldError(failure, "reason")}><textarea className="input" required maxLength={1000} value={reason} onChange={e => setReason(e.target.value)} /></Field> : null}
  </FormDialog>;
}
...
export function ContractWorkflow({ projectId, detail, codes, codesReady, reading, unavailable, onChanged, onRefresh }: { projectId: string; detail: ContractDetail; codes: CostCode[]; codesReady: boolean; reading: boolean; unavailable: boolean; onChanged: () => Promise<void>; onRefresh: () => Promise<void> }) {
  ...
  async function save(body: Record<string, unknown>) {
    ...
    try {
      ...
      else if (editor.kind === "signed") await construction.registerSignedContract(projectId, detail.id, String(body.reason));
  ...
  const editorBlocker = unavailable ? "The contract could not be refreshed. Cancel this form, then refresh before saving." : !editor ? null : editor.kind === "header" || editor.kind === "line" ? workflow.editing_blocker : ({ signed: workflow.signed_registration_blocker, submit: workflow.submission_blocker, activate: workflow.activation_blocker, cancel: workflow.cancellation_blocker, complete: workflow.completion_blocker, terminate: workflow.termination_blocker })[editor.kind];
  const actions: [Action, string | null][] = detail.status === "draft" ? [["signed", workflow.signed_registration_blocker], ["submit", workflow.submission_blocker], ["cancel", workflow.cancellation_blocker]] : detail.status === "submitted" ? [["signed", workflow.signed_registration_blocker], ["activate", workflow.activation_blocker], ["cancel", workflow.cancellation_blocker]] : detail.status === "active" ? [["complete", workflow.completion_blocker], ["terminate", workflow.termination_blocker]] : [];
  return <div className="stack">
    {notice ? <Notice tone="success">{notice}</Notice> : null}
    {detail.status === "draft" || detail.status === "submitted" || detail.status === "cancelled" ? <Notice tone="info">If this agreement is already signed, choose Activate signed contract. For an unsigned draft, use the separate authorization workflow.</Notice> : <Notice tone="info">This contract carries a standing commitment. Completion or termination does not reduce the money committed.</Notice>}
    ...
    {codesReady && !codes.some(code => code.is_active) ? <Notice tone="warning">No active cost codes are available. New signed contracts receive their own contract allocation automatically.</Notice> : null}
    ...
    <Card title="Contract lines" description="Each line allocates original value; commitment and certification remain grouped by cost code in Position."><TableScroll label="Draft contract lines"><thead><tr><th scope="col">Line</th><th scope="col">Description / cost code</th><th scope="col">Value excluding tax</th><th scope="col">Action</th></tr></thead><tbody>{detail.lines.map(line => <tr key={line.id}><th scope="row">{line.sequence}</th><td>{line.description}<p>{line.cost_code}</p>{line.notes}</td><td>{money(line.original_amount_ex_tax, detail.currency_code)}</td><td>{!workflow.editing_blocker ? <Button disabled={unavailable || busy || reading || !codesReady} onClick={() => begin({ kind: "line", line })}>Edit line {line.sequence}</Button> : "Read-only"}</td></tr>)}</tbody></TableScroll>{!detail.lines.length ? <p>No lines yet. Activating an already signed agreement creates its contract allocation automatically.</p> : null}</Card>
