...
import { Icon, Button, KeyValue, KeyValueGrid, StatusDot } from "@/components/ui";
    ...
    {parcels.map(parcel => <article className="parcel-estate parcel-architectural" key={parcel.id}>
      <div className="parcel-estate-main">
        <header className="parcel-estate-identity">
          <div className="parcel-estate-kicker"><span className="parcel-classification"><Icon name="land" />Land holding · registered parcel</span><StatusDot tone={parcel.is_active ? "success" : "muted"}>{parcel.is_active ? "Active" : "Inactive"}</StatusDot></div>
        ...
        <div className="parcel-estate-facts"><h3 className="property-section-title"><Icon name="access" />Title & ownership</h3><KeyValueGrid columns={2}>
        ...
        <footer className="parcel-estate-footer"><span><Icon name="documents" />Title, site and supporting records</span><Button variant="primary" onClick={() => onOpen(parcel)}>Open parcel {parcel.plot_number}</Button></footer>
      </div>
      <aside className="parcel-estate-context">
        {canSeeCost && parcel.financials_visible !== false ? <section><span className="eyebrow"><Icon name="money" />Acquisition</span><p className="parcel-estate-price">{money(parcel.purchase_price, parcel.base_currency_code)}</p><p className="footnote">Recorded purchase price · fees separate</p><div className="parcel-estate-date"><span>Acquired</span><strong>{businessDate(parcel.acquisition_date)}</strong></div></section> : <section><span className="eyebrow"><Icon name="money" />Acquisition</span><p>{businessDate(parcel.acquisition_date)}</p></section>}
        <section><span className="eyebrow"><Icon name="blueprint" />Planning context</span><h3>{parcel.zoning ?? "Zoning not established"}</h3><p className="footnote">Recorded classification. Planning limits and source evidence are held in the parcel record.</p></section>
      </aside>
    </article>)}
  </div>;
}
