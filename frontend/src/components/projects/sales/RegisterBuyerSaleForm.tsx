...
export function RegisterBuyerSaleForm({ projectId, unitId, clientId, unitOption, onSaved, onCancel }: {
  ...
  return <DraftBoundary dirty={Boolean(agentId || price !== unitOption.reference_price_ex_tax || form.name || form.phone || form.email || form.reason || form.date || form.client !== (clientId ?? "new"))} busy={busy}>
    <form onSubmit={async event => {
      ...
      try {
        const result = await sales.registerBuyer(projectId, {
          ...
          ...(form.client === "new" ? { buyer: { agent_id: agentId || null, display_name: form.name.trim(), sole_purchaser_name: form.name.trim(),
      ...
      {form.client === "new" ? <AgentSelect projectId={projectId} value={agentId} onChange={setAgentId} disabled={busy} /> : <p className="field-hint">The selected buyer&apos;s Agent association is copied into this transaction. Change it in Buyers for future reservations.</p>}
