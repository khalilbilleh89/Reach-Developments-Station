...
import { Badge, Button, ButtonRow, Drawer, Field, FieldRow, FormSection, KeyValue, KeyValueGrid, Notice } from "@/components/ui";
...
function ActionForm({ project, initial, source, onSaved, onCancel }: { project: string; initial?: ManagementAction; source: ActionSource; onSaved: (action: ManagementAction) => void; onCancel: () => void }) {
  ...
  return <form onSubmit={save} className="stack"><FormSection title="Management commitment" description="A named owner and a dated next step. This action does not change its source record.">
    <Field label="Title"><input className="input" required maxLength={200} value={title} onChange={(event) => setTitle(event.target.value)} /></Field>
    <Field label="Description" optional><textarea className="input" maxLength={2000} rows={4} value={description} onChange={(event) => setDescription(event.target.value)} /></Field>
    <FieldRow><Field label="Owner"><select className="input" required value={owner} onChange={(event) => setOwner(event.target.value)}><option value="">Select an eligible owner</option>{owner && owners.status === "ready" && !owners.data.some((user) => user.user_id === owner) ? <option value={owner}>{initial?.owner.user_id === owner ? initial.owner.display_name : "Selected owner"}</option> : null}{owners.status === "ready" ? owners.data.map((user) => <option key={user.user_id} value={user.user_id}>{user.display_name}</option>) : null}</select></Field>
      <Field label="Due date"><input className="input" required type="date" value={due} onChange={(event) => setDue(event.target.value)} /></Field></FieldRow>
    ...
    {initial ? <Field label="Reason" optional hint="Required for due-date changes and reassignment after work starts."><textarea className="input" maxLength={2000} value={reason} onChange={(event) => setReason(event.target.value)} /></Field> : null}
  </FormSection>
...
export function ActionRecord({ id, canWrite, onClose, onChanged }: { id: string; canWrite: boolean; onClose: () => void; onChanged: () => void }) {
  ...
  return <Drawer title={action?.title ?? "Management action"} subtitle={action ? `${action.project_code} · ${action.project_name}` : undefined}
    ...
    {answer.status === "failed" ? <Notice tone="error">{answer.message}</Notice> : !action ? <ReadState answer={answer} label="Reading action…" /> : tab === "action" ? <div className="stack">
      {editing && canWrite ? <ActionForm key={action.version} project={action.project_id} initial={action} source={action} onSaved={refresh} onCancel={refresh} /> : <>
        ...
        {canWrite ? <><Field label="Transition reason" optional hint="Required to cancel or reopen a completed/cancelled action."><textarea className="input" maxLength={2000} value={reason} onChange={(event) => setReason(event.target.value)} /></Field><ButtonRow>
