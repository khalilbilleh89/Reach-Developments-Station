...
function PortfolioScreen() {
  ...
  return <AppShell area="portfolio" user={state.user} crumbs={[{ label: "Portfolio", href: "/portfolio/" }, { label: id ? "Project management summary" : section }]}>
    ...
    {!allowed ? <Notice tone="info">Portfolio is not available to your role.</Notice> : projectDetail && id ? <PortfolioProject key={id} id={id} /> : <>
      ...
      <TabPanel group="portfolio-sections" tab={section}>{section === "reporting" ? <Reporting canWrite={canWrite} /> : section === "projects" ? <PortfolioProjects /> : section === "outlook" ? <PortfolioOutlook /> : section === "actions" ? <Actions canWrite={canWrite} /> : section === "exceptions" ? <div className="stack"><SectionHeader title="Source risks" /><PortfolioRisks /><SectionHeader title="Overdue management actions" /><Notice tone="info">Action lateness is separate from source risk severity. Completing an action does not resolve a risk.</Notice><Actions canWrite={canWrite} overdueOnly /></div> : <PortfolioOverview />}</TabPanel>
    </>}
  </AppShell>;
}
