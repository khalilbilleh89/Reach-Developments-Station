...
| `reference_values` | Shared and country-scoped controlled vocabularies outside project inventory choices. |
| `inventory_options` | Project-specific inventory vocabulary, administered through Inventory Configuration before a batch; never supplied by the bundle. |
...
| `management_report_snapshots` | Prospective immutable management captures; no backfill or legacy intake. |
| `management_report_snapshot_projects` | Immutable authorization scope of those captures; never imported. |
