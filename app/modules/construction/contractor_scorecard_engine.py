...
on_time_milestones
    Completed milestones not preceded by a delayed status
    (approximated: completed_milestones - delayed_milestones when both
    statuses cannot coexist; i.e. milestones that reached completed
    directly without being delayed).
...
on_time_completion_ratio
    on_time_milestones / completed_milestones.  None if no completions.
overrun_ratio
...
high_risk_alert_count
    Caller-supplied count of current unresolved HIGH alerts for this contractor.
...
risk_score
    Derived from high_risk_alert_count (10 points deducted per alert, floor 0).
performance_score
...
class ContractorScorecardInput:
    ...
    high_risk_alert_count:
        Current count of unresolved HIGH-severity risk alerts for this
        contractor (pre-computed by the caller from risk alert engine).
    """
...
class ContractorScorecard:
    ...
    on_time_milestones:
        Completed milestones that were never delayed (completed_milestones
        minus those that appear delayed; since a milestone can only have one
        current status, this equals completed_milestones when no milestone
        is simultaneously completed and delayed).
    over_budget_milestones:
    ...
    on_time_completion_ratio:
        on_time_milestones / completed_milestones.  None when no completions.
    overrun_ratio:
    ...
    completed_packages:
        Packages with status ``completed``.
    high_risk_alert_count:
        Current unresolved HIGH alert count (caller-supplied).
    schedule_score:
        Score 0–100 derived from delay_ratio.
    cost_score:
        Score 0–100 derived from overrun_ratio and avg cost variance.
    risk_score:
        Score 0–100 derived from high_risk_alert_count.
    performance_score:
...
def _compute_risk_score(high_risk_alert_count: int) -> float:
    """Compute risk score 0–100 from count of HIGH alerts."""
    return max(0.0, 100.0 - high_risk_alert_count * ALERT_PENALTY_POINTS)
...
def _compute_metrics(
    ...
    # on_time: completed and not in delayed state (statuses are mutually exclusive)
    on_time = completed  # delayed status cannot also be completed in the same record
    ...
    on_time_ratio: Optional[float] = on_time / completed if completed > 0 else None
...
def compute_contractor_scorecard(
    ...
    risk_score = _compute_risk_score(data.high_risk_alert_count)
    ...
    return ContractorScorecard(
        ...
        on_time_completion_ratio=metrics["on_time_ratio"],
        ...
        high_risk_alert_count=data.high_risk_alert_count,
...
def compute_contractor_trend(
    ...
    # Also include delayed milestones without completion_date in the last period
    # (they are active so we can't assign them to a completed period)
