...
export function isDevelopmentUiOnly(event, paths) {
  return event.name === 'pull_request' && event.base === 'main' &&
    event.head === 'eng/architectural-property-workspace' && event.repository === repository &&
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  let paths = [];
  if (event.name === 'pull_request' && event.base === 'main' && event.head === 'eng/architectural-property-workspace') {
