...
def test_create_building(client: TestClient):
    """POST /api/v1/buildings should create and return a building."""
    project_id = _create_project(client)
    phase_id = _create_phase(client, project_id)
    response = client.post(
        "/api/v1/buildings",
        json={"phase_id": phase_id, "name": "Block A", "code": "BLK-A"},
    )
...
def test_create_building_invalid_phase(client: TestClient):
    """POST /api/v1/buildings with non-existent phase_id should return 404."""
    response = client.post(
        "/api/v1/buildings",
        json={"phase_id": "no-such-phase", "name": "Block A", "code": "BLK-A"},
    )
...
def test_create_building_duplicate_code_in_phase(client: TestClient):
    ...
    project_id = _create_project(client)
    phase_id = _create_phase(client, project_id)
    client.post("/api/v1/buildings", json={"phase_id": phase_id, "name": "Block A", "code": "BLK-A"})
    response = client.post("/api/v1/buildings", json={"phase_id": phase_id, "name": "Block A2", "code": "BLK-A"})
    assert response.status_code == 409


def test_list_buildings_filtered_by_phase(client: TestClient):
    """GET /api/v1/buildings?phase_id=... should return buildings for that phase."""
    project_id = _create_project(client)
    phase_id = _create_phase(client, project_id)
    client.post("/api/v1/buildings", json={"phase_id": phase_id, "name": "Block A", "code": "BLK-A"})
    response = client.get(f"/api/v1/buildings?phase_id={phase_id}")
...
def test_update_building(client: TestClient):
    ...
    project_id = _create_project(client)
    phase_id = _create_phase(client, project_id)
    create = client.post("/api/v1/buildings", json={"phase_id": phase_id, "name": "Block A", "code": "BLK-A"})
